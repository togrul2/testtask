...
@app.post("/signup", response_model=schemas.Token, summary="User Signup")
...
@app.post("/login", response_model=schemas.Token, summary="User Login")
...
@app.post("/addpost", response_model=schemas.PostResponse, summary="Add a Post")
async def add_post(
...
@app.get("/getposts", response_model=list[schemas.PostResponse], summary="Get User Posts")
def get_posts(
        current_user: models.User = Depends(auth.get_current_user),
        db: Session = Depends(get_db),
):
...
@app.delete("/deletepost", summary="Delete a Post")
def delete_post(
        post_id: int,
        current_user: models.User = Depends(auth.get_current_user),
        db: Session = Depends(get_db),
